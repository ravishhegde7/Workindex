...
(function(){
  ...
  function viewChat(cid, en, cn, rt) {
    ...
    api('chats/' + cid + '/messages').then(function(d) {
      // Backend stores messages inside chat.messages array OR as separate Message docs
      var msgs = d.messages || [];
      if (!msgs.length) {
      ...
      g('chDrMsgs').innerHTML = msgs.map(function(m) {
        var senderName = (m.sender && (m.sender.name || m.sender)) || 'Unknown';
        var senderRole = (m.sender && m.sender.role) || (m.senderRole) || '';
        var isAdmin  = m.isAdminMessage || senderRole === 'admin';
        var isExpert = senderRole === 'expert';
        ...
        return '<div class="cmsg ' + cls + '">' +
          ...
          '<div style="margin-top:3px">' + esc(m.text || m.message || '[media]') + '</div>' +
          '<div class="cmeta" style="margin-top:4px;text-align:right">' + fmtT(m.createdAt) + '</div>' +
          '</div>';
      }).join('');
      g('chDrMsgs').scrollTop = g('chDrMsgs').scrollHeight;
    }).catch(function(err) {
      g('chDrMsgs').innerHTML = '<p style="text-align:center;color:#ef4444;padding:20px">Error loading messages: ' + err + '</p>';
    });
  }
  ...
  function renderHeatmapIndia(byState, total) {
    // Use SVG choropleth map
    g('hmMapArea').innerHTML = renderIndiaSvgMap(byState, total);
    setTimeout(attachSvgMapEvents, 100);
    return;
    /*
    var maxVal = 0;
  ...
  */ }
  ...
  function loadSettingsTab() {
    // Show API URL
    if (g('settApiUrl')) g('settApiUrl').value = API_URL;
    // Show admin info
    if (g('settAdminInfo')) {
      g('settAdminInfo').innerHTML =
        '<strong>' + esc(state.adminId||'admin') + '</strong>' +
        '<div style="color:#606078;margin-top:4px;font-size:12px">Last login: ' + (state.lastLogin ? new Date(state.lastLogin).toLocaleString('en-IN') : 'N/A') + '</div>';
    }
    // Load platform stats
    loadSettingsStats();
    // Password change
  ...
  function loadSettingsStats() {
    ...
    api('stats').then(function(d) {
      var s = d.stats||d;
      g('settStats').innerHTML =
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">' +
        ['Experts','Clients','Requests','Approaches','Chats','Credits Issued'].map(function(label, i) {
          var vals = [s.experts,s.clients,s.requests||s.posts,s.approaches,s.chats,s.totalCredits||s.creditsIssued||0];
          return '<div style="background:#18181d;border-radius:8px;padding:10px"><div style="font-size:10px;color:#606078;text-transform:uppercase;margin-bottom:4px">' + label + '</div><div style="font-size:20px;font-weight:800">' + (vals[i]||0) + '</div></div>';
        }).join('') +
        '</div>';
    ...
    var svgPaths = INDIA_STATES_SVG.states.map(function(st) {
      ...
      if (!data) {
        Object.keys(stateData).forEach(function(k) {
          if (k.toLowerCase().includes(st.name.toLowerCase().split(' ')[0])) data = stateData[k];
        });
      }
